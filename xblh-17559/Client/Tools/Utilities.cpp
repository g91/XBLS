...
namespace xbox {
	namespace utilities {
		...
		void patch_BLOCK_LIVE(BOOL enable) {
			//if (global::isDevkit) {
			//	//xbox::utilities::DbgOut("[xblh] * Blocking xbox live DNS\r\n");
			...
			//	if (!offsets) {
			//		//xbox::utilities::DbgOut("[xblh] Failed to load dns offsets!\r\n");
			//		return;
			//	}

			//	char* SIFLC = "SIFLC.%sXBOXLIVE.COM\0";
			//	char* PIFLC = "PIFLC.%sXBOXLIVE.COM\0";
			//	char* NOTICE = "NOTICE.%sXBOXLIVE.COM\0";
			//	char* XEXDS = "XEXDS.%sXBOXLIVE.COM\0";
			//	char* XETGS = "XETGS.%sXBOXLIVE.COM\0";
			//	char* XEAS = "XEAS.%sXBOXLIVE.COM\0";
			//	char* XEMACS = "XEMACS.%sXBOXLIVE.COM\0";

			//	if (enable) {
			//		//null out xbox live dns tags
			//		if (offsets->live_siflc)//FIXME: check the others
			//			xbox::utilities::setMemory((VOID*)offsets->live_siflc, (VOID*)nullStr, nullStrSize);

			//		xbox::utilities::setMemory((VOID*)offsets->live_piflc, (VOID*)nullStr, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_notice, (VOID*)nullStr, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_xexds, (VOID*)nullStr, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_xetgs, (VOID*)nullStr, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_xeas, (VOID*)nullStr, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_xemacs, (VOID*)nullStr, nullStrSize);
			//	} else {
			//		//null out xbox live dns tags
			//		if (offsets->live_siflc)//FIXME: check the others
			//			xbox::utilities::setMemory((VOID*)offsets->live_piflc, (VOID*)SIFLC, nullStrSize);

			//		xbox::utilities::setMemory((VOID*)offsets->live_piflc, (VOID*)PIFLC, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_notice, (VOID*)NOTICE, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_xexds, (VOID*)XEXDS, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_xetgs, (VOID*)XETGS, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_xeas, (VOID*)XEAS, nullStrSize);
			//		xbox::utilities::setMemory((VOID*)offsets->live_xemacs, (VOID*)XEMACS, nullStrSize);
			//	}
			//}
		}
		...
		void DbgOut(const char* text, ...)
		{
			...
			std::ofstream log_file("Client:\\XBLH.log", std::ios_base::app);
			if (log_file.is_open())
				log_file << dest;
			else
				return;
		...
		//		if (sec) {
		//			PIMAGE_EXPORT_ADDRESS_TABLE exports = (IMAGE_EXPORT_ADDRESS_TABLE)sec->ImageInfo.ExportTableAddress;
		//			if (exports)
		...
		BOOL ProcessCPUKeyBin() {
			Sleep(50);
			//xbox::utilities::DbgOut("[xblh] searching for CPU key spoof\n");
			if (fileExists("Client:\\CPUKey.bin")) {
				////xbox::utilities::DbgOut("[xblh] CPUKey Found at Client:\\CPUKey.bin\n");

				MemoryBuffer mbCpu;
				if (!readFile("Client:\\CPUKey.bin", mbCpu)) {
					//xbox::utilities::DbgOut("[xblh] CPUKey Not Loaded at Client:\\CPUKey.bin\n");
					return FALSE;
				}

				memcpy(data::cpuKey, mbCpu.GetData(), 0x10);
			} else {
				//xbox::utilities::DbgOut("[xblh] CPUKey not Found at Client:\\CPUKey.bin\n");
				return FALSE;
			}

			Sleep(50);
			//xbox::utilities::DbgOut("[xblh] setting up CPUKey data\n");
		...
		BOOL ProcessKVBin() {
			Sleep(50);
			xbox::utilities::DbgOut("[xblh] searching for KeyVault spoof\n");
			if (fileExists("Client:\\KV.bin")) {
				//xbox::utilities::DbgOut("[xblh] KV Found at Client:\\KV.bin\n");

				MemoryBuffer mbKV;
				if (!readFile("Client:\\KV.bin", mbKV)) {
					//xbox::utilities::DbgOut("[xblh] KV Not Loaded at Client:\\KV.bin\n");
					return FALSE;
				}

				////xbox::utilities::DbgOut("[xblh] setting up KeyVault data\n");
				memcpy(&data::buffer, mbKV.GetData(), sizeof(KEY_VAULT));
				return TRUE;
			} else {
				//xbox::utilities::DbgOut("[xblh] KV not Found at Client:\\KV.bin\n");
				return FALSE;
			}
		...
		BOOL SetKeyVault() {
			...
			//xbox::utilities::DbgOut("[xblh] Starting KeyVault Mounting\n");
			...
			//xbox::utilities::DbgOut("[xblh] KeyVault Mounted\n");
			return TRUE;
		}
	}
}
...
//PLDR_DATA_TABLE_ENTRY hXam = (PLDR_DATA_TABLE_ENTRY)global::modules::xam;
//PLDR_DATA_TABLE_ENTRY hKernel = (PLDR_DATA_TABLE_ENTRY)global::modules::krnl;
...
//		xbox::utilities::writeFile("Client:\\xam.bin", (PBYTE)&securityInfo->AllowedMediaTypes, size);
//	}
...
//			xbox::utilities::writeFile("Client:\\kernel.bin", (PBYTE)&securityInfo->AllowedMediaTypes, size);
//	}
//}
...
/*		data::type1KV = TRUE;
		...
		//	//xbox::utilities::DbgOut("The cpu key provided is not for this keyvault.");