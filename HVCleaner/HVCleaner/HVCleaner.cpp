// HVCleaner.cpp : Defines the entry point for the application.
//
...
DWORD WINAPI ServerThread(PVOID pParam)
{
	...
	while (TRUE)
	{
		...
		{
			PBYTE data = new BYTE[0x100];
			ZeroMemory(data, 0x100);

			BYTE pSalt[0x10];
			memcpy((char*)Buffer + 18, pSalt, 0x10);
			xbox::Cleaning::CallXamExecuteChallenge(pSalt);

			sendto(sock, (char *)data, 0x100, 0, (struct sockaddr *)&client, client_length);
			delete data;
		}
	}
